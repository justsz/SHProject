...
MIN_RACE_COUNT = 10
RAND = random.Random(456) #number in brackets is seed. leave brackets empty to have a different string of random numbers each time
...
def initializeScores (initType, keepHistory, keepGeneratedScore, mean, std, participants):
    """General function for score initialization."""
    people = {}
...
def getResults (race, people):
    """Returns an intermediate results representation."""
    results = []
    for runner in race:
        results.append([people[runner[0]], runner[1]]) #person, RT
    return results



def shuffle (races):
    """Shuffle a list from the same generator as used for other tasks."""
    RAND.shuffle(races)



def cropResultsAndCalcMeansAndStds (results, cropType, cropLimit, givePointsToCropped, minSP):
    """Applies a cutoff rule, then calculates MT, ST, MP and SP. A minimum value for SP can be set."""
    MP = 0.0
    MT = 0.0
    SP = 0.0
    ST = 0.0
    ...
    if (cropType == 'normal'):
        ...
        print "Unsupported crop type. Exiting."
        exit()
...
def updateRunnerScores (results, meansAndStds, people, keepHistory):
    """Using race results and MT, ST, MP and SP update runners' scores."""
    for res in results:
...
def dropIntermediateHistory (people, keepHistory, keepGeneratedScore):
    """Drop all scores from runners' histories. Can keep their current rank as a first race score."""
    if keepHistory:
...
def getMeanAndStdOfScores (people, keepHistory, minRaceCount, dropIntermediateScores, run):
    """Returns the mean and standard deviation of all scores from players with some minimum amount of REAL races (not ones generated by reruns)."""
    minRC = minRaceCount
...
def getIndividualMeansAndStds (people, keepHistory, minRaceCount, dropIntermediateScores, run):
    """Returns the individual means and standard deviations of all players' scores that have some minimum amount of REAL races (not ones generated by reruns)."""
    minRC = minRaceCount
    if not dropIntermediateScores:
        minRC *= run
...
def rebaseScores (people, newMean, newStd, minRaceCount):
    """Rebase the means of scores of all players with some minimum race count to a new mean and standard deviation."""
