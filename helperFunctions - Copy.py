...
def getResults (race, people):
    results = []
    for runner in race:
        results.append([runner[0], people[runner[0]][0], runner[1]]) #id, prevRank, RT
    return results
...
def cropResultsAndCalcMeansAndStds (results, cropType, cropLimit, givePointsToCropped):
    MP = 0.0
    MT = 0.0
    SP = 0.0
    ST = 0.0
    ...
    if (cropType == 'normal'):
        ...
        if givePointsToCropped:
            total = 0
            for j in results:
                if abs(j[2] - initMean) < maxDeviation:
                    MP = MP + j[1]
                    MT = MT + j[2]
                    total += 1
            ...
            results[:] = [r for r in results if abs(r[2] - initMean) < maxDeviation]
...
def updateRunnerScores (results, meansAndStds, people, keepHistory):
    for res in results:
        ...
        RP = meansAndStds[0] + meansAndStds[1] * (meansAndStds[2] - res[2]) / meansAndStds[3]
...
def getMeanAndStdOfScores (people, keepHistory, minRaceCount, dropIntermediateScores, run):
    ...
    std = -1
    if keepHistory:
        mean = numpy.mean([v[0] for k,v in people.iteritems() if len(v[1]) >= minRC])
        std = numpy.std([v[0] for k,v in people.iteritems() if len(v[1]) >= minRC])
    else:
...
def getIndividualMeansAndStds (people, keepHistory, minRaceCount, dropIntermediateScores, run):
    ...
    if keepHistory:
        means = [numpy.mean(v[1]) for k,v in people.iteritems() if len(v[1]) >= minRC]
        stds = [numpy.std(v[1]) for k,v in people.iteritems() if len(v[1]) >= minRC]
        return means, stds
    else:
        means = [v[0] for k,v in people.iteritems() if v[1] >= minRaceCount]
        return means, [-1]
